	...
	<div class="container">
        ...
        <input name="memberNo" type="hidden" value="${nowMember.getMemberNo()}">
				...
				<div class="d-flex mb-5 mt-3">
					<div class="col">
						<span class="inputGroupText">파일</span> <span id="download"
        ...
        <textarea readonly class="form-control mt-3 mb-3" style="background-color:white; height:10em;">${assignment.content}</textarea>
				<div class="d-flex justify-content-around mt-3 mb-3">
					...
					<div class="col-8 d-flex justify-content-end">
						<label class="inputGroupText" for="enddate">마감일</label><span
							class="font-weight-bold" style="font-size: 1.2em;">${assignment.deadline}</span>
					</div>
				</div>
			</div>
			<!-- 과제 제출물이 없을때 -->
			<c:if test="${empty assignmentSubmit}">
				<form action='../assignmentSubmit/add' method='post'
					enctype='multipart/form-data'>
					<input name='memberNo' type='hidden' value='${nowMember.memberNo}'>
					<input name='assignmentNo' type='hidden'
						value='${assignment.assignmentNo}'> <label
						class="inputGroupText mt-5">과제제출</label>
					<textarea id="student_content" class="form-control" name='content' style="height:10em;"></textarea>
					<div class="d-flex justify-content-between mt-5 mb-5">
						<div id="file-input">
							<label class="inputGroupText">파일첨부</label><input
								class="btn btn-sm btn-light" name='partfile' type='file'>
						</div>
			...
			<c:if test="${not empty assignmentSubmit}">
				<form action='../assignmentSubmit/update' method='post'
					enctype='multipart/form-data'>
					<input name='memberNo' type='hidden' value='${nowMember.memberNo}'>
					<input name='assignmentNo' type='hidden'
						value='${assignment.assignmentNo}'> <label
					...
					<label class="inputGroupText mt-3">제출한 파일</label><span
						id="submit-download" title="다운로드"
						style="cursor: pointer; font-size: 1.2em;">${assignmentSubmit.fileVO.originalName}</span>
					<div class="d-flex justify-content-between mt-3 mb-5">
						<div id="file-input">
							<label class="inputGroupText">파일변경</label><input
								class="btn btn-sm btn-light" name='partfile' type='file'>
						</div>
