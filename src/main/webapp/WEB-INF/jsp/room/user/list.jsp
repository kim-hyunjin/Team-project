			...
			<ul class="teacher_list user_list">
				<c:forEach items="${teachers}" var="teacher">
					<li class="user_card" onclick="showUserDetail(${teacher.user.userNo},${teacher.memberNo})">
						...
						<span data-toggle="modal" data-target="#MemberDetailModal"  style="cursor: pointer;">${teacher.user.name}</span>
					</li>
				</c:forEach>
			</ul>
		</div>
		...
		<div class="student" style="margin-top: 10em;">
			...
			<ul class="student_list user_list">
				...
				<li class="user_card" onclick="showUserDetail(${student.user.userNo}, ${student.memberNo})">
					...
					<span data-toggle="modal" data-target="#MemberDetailModal"  style="cursor: pointer;">${student.user.name}</span>
				</li>
				</c:forEach>
			</ul>
		</div>
...
<!-- Modal -->
<!-- 사용자 detail -->
<div class="modal fade" id="MemberDetailModal" tabindex="-1" role="dialog"
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
          ...
          <div class="modal-body">
                <form id="modal-memberDetail" class="inputGroup">
                    <input name='memberNo' type='hidden'> 
                    <input name='email' type='text' readonly placeholder="이메일">
                    <input name='name' type='text' readonly placeholder="이름">
                    <input name='tel' type='text' readonly placeholder="전화번호">
                    <input name='major' type='text' readonly placeholder="전공">
                    <input name='introduce' type='text' readonly placeholder="소개">
                    <input name='createDate' type='text' readonly placeholder="회원가입일">
                </form>
            </div>
            <div class="modal-footer">
                <c:if test="${nowMember.role == 0}">
                <button type="button" class="btn btn-danger" id="button_event" 
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <button id="memberAddModalClose" type="button" class="close" data-dismiss="modal"
                    aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
          <div class="modal-body">
                 <form id='modalMemberAddForm' action='add' method='post' class="inputGroup">
			        <input name="class_no" type="hidden" value="${clazzNowNo}">
			        <input name="role" type="hidden">
			        <input type="email" name="email" id="emailInput" placeholder="초대 대상의 이메일을 입력하세요." />
			    </form>
            </div>
            <div class="modal-footer">
                <button type="button" class="btn btn-primary" id="memberAddBtn">Submit</button>
            </div>
        </div>
    </div>
</div>
