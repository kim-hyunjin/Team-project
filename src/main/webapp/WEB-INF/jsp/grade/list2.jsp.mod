...
<div class="room_contents">
	<div class="container-fluid">
		<div class="d-flex">
			<div class="col-2" style="position:fixed; background-color:white;">
				<select id="filter_list" onclick="activeFilter(this.value)">
					<option>전체</option>
					<option v-for="item in assignments">{{item.title}}</option>
					<option>제출</option>
					<option>미제출</option>
				</select>
			</div>

			<div class="col-10" id="grade_contents">
				<ul id="grade_list">
					<li v-for="user in users" v-if="user.role != 0"><span
						class="user_info">{{user.user.name}}</span>
						<div v-for="submit in submits">
							<div
								v-if="submit.user.userNo == user.userNo && submit.hasOwnProperty('createDate')"
								class="assignments_info green">
								<a
									v-bind:href="`../assignmentSubmit/submitted?assignmentNo=`+submit.assignmentNo"
									title="제출 과제 확인 및 평가하기">
